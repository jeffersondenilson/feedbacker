...
describe('<ModalCreateAccount />', () => {
  ...
  it('should render modal-create-account correctly', async () => {
    router.push('/')
    await router.isReady()
    const wrapper = shallowMount(ModalCreateAccount, {
      global: {
        plugins: [
          router,
  ...
  it('should set correct value in name input', async () => {
    const name = 'name'
    useField.mockReturnValue({ value: name, errorMessage: '' })
    router.push('/')
    await router.isReady()
    const wrapper = shallowMount(ModalCreateAccount, {
      global: {
    ...
    const nameErrorMessage = wrapper.find('[data-test=name-error-message]')
    await nameInput.setValue(name)

    expect(nameInput.element.value).toBe(name)
    expect(nameErrorMessage.exists()).toBe(false)
  })

  it('should show error with invalid name', async () => {
    const name = ''
    ...
    expect(nameInput.element.value).toBe(name)
    expect(nameInput.classes('border-brand-danger')).toBe(true)
    expect(nameErrorMessage.exists()).toBe(true)
    expect(nameErrorMessage.text()).toBe(errorMessage)
    expect(wrapper.html()).toMatchSnapshot()
  })
